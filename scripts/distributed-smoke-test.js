/**
 * 분산 환경 스모크 테스트 (10회 연속)
 * 
 * 8주차 요구사항: 분산 환경에서 스모크 10회 연속 OK
 * 
 * 사용법: npm run test:distributed
 */
...
function logIteration(iteration, passed, message) {
  const icon = passed ? '✅' : '❌'
  console.log(`${icon} 스모크 테스트 ${iteration}/${NUM_ITERATIONS}: ${passed ? 'PASS' : 'FAIL'}`)
...
async function runSmokeTest(iteration) {
  try {
    // 1. Health Check
    ...
    const pollRes = await axios.post(`${API_BASE_URL}/polls`, {
      ...
      title: `Distributed Test Poll ${iteration}`,
      description: `Distributed Test Description ${iteration}`,
      candidates: [
        { id: '1', label: 'Option 1' },
        { id: '2', label: 'Option 2' }
      ],
    ...
    const registerRes = await axios.post(`${API_BASE_URL}/user/register`, {
      name: `Distributed User ${iteration}`,
      walletAddress: `0x${(iteration + 2000).toString().padStart(40, '0')}`,
      studentId: `2024${String(iteration + 2000).padStart(4, '0')}`
    }, { timeout: 10000 })
    ...
    const voteRes = await axios.post(`${API_BASE_URL}/vote/create`, {
      ...
      candidate: 'Option 1',
      pollId,
      nullifierHash: `distributed-nullifier-${iteration}`
    }, { timeout: 10000 })
    ...
    try {
      await axios.post(`${API_BASE_URL}/vote/create`, {
        ...
        candidate: 'Option 2',
        pollId,
        nullifierHash: `distributed-nullifier-${iteration}` // 같은 nullifierHash
      }, { timeout: 10000 })
      throw new Error('중복 투표가 허용됨 (409가 발생해야 함)')
    } catch (error) {
      if (error.response && error.response.status === 409) {
        // 정상: 중복 차단됨
      } else if (error.message.includes('중복 투표가 허용됨')) {
        throw error
      } else {
        // 다른 오류는 무시 (중복 차단 확인이 목적)
      }
    }
...
async function runAllTests() {
  console.log('🚀 분산 환경 스모크 테스트 시작 (10회 연속)')
  ...
  for (let i = 1; i <= NUM_ITERATIONS; i++) {
    try {
      const passed = await runSmokeTest(i)
  ...
  // 결과 요약
  console.log('\n' + '='.repeat(50))
  console.log('📊 분산 환경 스모크 테스트 결과 요약')
